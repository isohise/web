...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function displayMeals(category, filter = null) {
        ...
        const filteredMeals = filter ? meals.filter(meal => meal.category === category && meal.kind === filter) : meals.filter(meal => meal.category === category);
    ...
    document.querySelectorAll('.filters button').forEach(button => {
        button.addEventListener('click', function() {
            const categorySection = this.closest('section').querySelector('.meals-grid').id.split('-')[0];
            const kind = this.getAttribute('data-kind');
    
            this.closest('.filters').querySelectorAll('button').forEach(btn => btn.classList.remove('active'));
